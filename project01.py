...
- Gộp với dữ liệu của file overview_reviews.xlsx để tạo thành bộ dữ liệu mới''')
    ...
    st.write('Nhận xét: Các chủ đều phản ánh các đánh giá tích cực của ứng viên/nhân viên về lương, nơi làm việc, sếp, đồng nghiệp, phát triển. Tuy nhiên, thứ tự của cá yếu tố trên ở mỗi chủ đề lại có sự khác nhau, cho thấy mức độ ưu tiên trong đánh giá của mỗi ứng viên/nhân viên.')
    st.write('-----------------------------------------------------')
